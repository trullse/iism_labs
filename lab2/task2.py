...
# Дискретное распределение (значения и их вероятности)
values = [1, 2, 3, 4, 5]
probabilities = [0.1, 0.2, 0.3, 0.25, 0.15]


def generate_discrete_random_value(values, probabilities):
    """Генерируем дискретную случайную величину по заданным вероятностям"""
    ...
    for value, prob in zip(values, probabilities):
        ...
        if x < cumulative_probability:
            return value
    return values[-1]  # на случай погрешности


def simulate_discrete_distribution(num_samples):
    """Симулируем генерацию случайной величины num_samples раз"""
...
def plot_histogram(samples):
    """Строим гистограмму выборки и сравниваем с теоретическим распределением"""
...
def run_simulation():
    try:
        ...
        # Вычисление точечной оценки и доверительного интервала
        ...
        # Запуск построения графика с задержкой, чтобы текст был виден до графика
